...
TaskPixel.TagDisplay = {
  ...
  // 创建内联标签管理器
  createInlineManager: function (options = {}) {
    try {
      const config = { ...this.defaultOptions, ...options };

      return {
        // 渲染任务标签的快捷方法
        renderTaskTags: (taskId, container, opts = {}) => {
          return this.renderTaskTags(taskId, container, { ...config, ...opts });
        },

        // 渲染目标标签的快捷方法
        renderGoalTags: (taskId, goalId, container, opts = {}) => {
          return this.renderGoalTags(taskId, goalId, container, {
            ...config,
            ...opts,
          });
        },
        ...
        // 创建标签输入和选择界面
        createTagInputInterface: (
          container,
          selectedTags = [],
          onUpdate = null
        ) => {
          const wrapper = document.createElement("div");
          wrapper.className = "tag-input-interface";

          // 创建选择器
          const selector = this.createTagSelector(selectedTags, config);

          // 如果提供了更新回调，绑定事件
          if (onUpdate) {
            selector.addEventListener("tagSelectionChange", (e) => {
              onUpdate(e.detail.selectedTags);
            });
          }
          ...
          return {
            getSelectedTags: () => this.getSelectedTags(selector),
            setSelectedTags: (tagIds) => this.setSelectedTags(selector, tagIds),
            destroy: () => {
      ...
      console.error(
        "TagDisplay.createInlineManager: 创建内联管理器失败",
        error
      );
      return null;
    }
  ...
  // 显示新标签创建对话框 (替代 TagSelector 功能)
  showCreateTagDialog: function (onSuccess) {
    return this._showCreateTagDialog(onSuccess);
  },
  createTagSelector: function (selectedTags = [], options = {}) {
    try {
      const config = {
        ...this.defaultOptions,
        ...options,
        allowRemove: false, // 选择器中不允许删除
        interactive: true,
      };

      const container = document.createElement("div");
      container.className = "tag-selector pixel-border p-4 bg-white";
      ...
      container.innerHTML = `
        ...
        <div class="tag-selection-area">
          <div class="tag-list flex flex-wrap gap-2 mb-3"></div>
        </div>
      `;

      // 渲染标签选项
      this._renderTagOptions(container, allTags, selectedTags, config);

      // 绑定选择器事件
      this._bindSelectorEvents(container, config);

      return container;
    } catch (error) {
      console.error("TagDisplay.createTagSelector: 创建选择器失败", error);
      return null;
    }
  },
  ...
  getSelectedTags: function (selectorContainer) {
    try {
      const checkboxes = selectorContainer.querySelectorAll(
        ".tag-checkbox:checked"
      );
      return Array.from(checkboxes).map((cb) => cb.value);
    } catch (error) {
      console.error("TagDisplay.getSelectedTags: 获取选中标签失败", error);
      return [];
    }
  },
  ...
  setSelectedTags: function (selectorContainer, tagIds) {
    try {
      const checkboxes = selectorContainer.querySelectorAll(".tag-checkbox");
      checkboxes.forEach((cb) => {
        cb.checked = tagIds.includes(cb.value);
        this._updateTagOptionAppearance(cb);
      });
      return true;
    } catch (error) {
      console.error("TagDisplay.setSelectedTags: 设置选中标签失败", error);
      return false;
    }
  },

  // 私有方法：渲染标签选项
  _renderTagOptions: function (container, allTags, selectedTags, config) {
    try {
      const tagList = container.querySelector(".tag-list");
      tagList.innerHTML = "";

      allTags.forEach((tag) => {
        const optionElement = this._createTagOption(
          tag,
      ...
      console.error("TagDisplay._renderTagOptions: 渲染选项失败", error);
    }
  },

  // 私有方法：创建标签选项
  _createTagOption: function (tag, isSelected, config) {
    try {
      const label = document.createElement("label");
      label.className = "tag-option flex items-center cursor-pointer";
      ...
      tagBadge.className =
        "tag-badge pixel-border px-3 py-1 text-sm font-bold font-display transition-all";

      // 应用样式
      this._updateTagOptionStyle(tagBadge, tag, isSelected);

      // 显示文本with fallback
      tagBadge.textContent = tag.display_text || "#" + tag.name || tag.id;
      ...
      label.addEventListener("click", (e) => {
        ...
        this._updateTagOptionAppearance(checkbox);
      });

      return label;
    } catch (error) {
      console.error("TagDisplay._createTagOption: 创建选项失败", error);
      return null;
    }
  },

  // 私有方法：更新标签选项外观
  _updateTagOptionAppearance: function (checkbox) {
    try {
      const tagBadge = checkbox.nextElementSibling;
      const tagId = checkbox.value;
      const tag = TaskPixel.TagManager.getTagById(tagId);

      if (tag && tagBadge) {
        this._updateTagOptionStyle(tagBadge, tag, checkbox.checked);
      }
    } catch (error) {
      console.error(
        "TagDisplay._updateTagOptionAppearance: 更新外观失败",
        error
      );
    }
  },
  ...
  // 私有方法：更新标签选项样式
  _updateTagOptionStyle: function (element, tag, isSelected) {
    try {
      const baseColor = tag.color || "#374151";
      ...
      if (isSelected) {
        element.style.cssText = `
          ...
          color: ${baseColor}; 
          background-color: ${this._hexToRgba(baseColor, 0.1)};
          box-shadow: 0 0 10px ${baseColor}40;
        `;
        element.classList.add("bg-pixel-accent", "shadow-pixel-glow");
        element.classList.remove("bg-white", "hover:bg-gray-50");
      } else {
        element.style.cssText = `
          border-color: ${baseColor}; 
          color: ${baseColor}; 
        ...
        `;
        element.classList.remove("bg-pixel-accent", "shadow-pixel-glow");
        element.classList.add("bg-white", "hover:bg-gray-50");
      }
    } catch (error) {
      console.error("TagDisplay._updateTagOptionStyle: 更新样式失败", error);
    }
  },

  // 私有方法：绑定选择器事件
  _bindSelectorEvents: function (container, config) {
    try {
      // 搜索功能
      const searchInput = container.querySelector("#tag-search-input");
      if (searchInput) {
        searchInput.addEventListener("input", (e) => {
          this._filterTagOptions(container, e.target.value.toLowerCase());
        });
      }
      ...
      if (createBtn) {
        createBtn.addEventListener("click", () => {
          this._showCreateTagDialog((newTag) => {
            // 重新渲染选项
            const allTags = TaskPixel.TagManager.getAllTags();
      ...
      container.addEventListener("change", (e) => {
        if (e.target.classList.contains("tag-checkbox")) {
          this._updateTagOptionAppearance(e.target);

          // 触发自定义事件
          const changeEvent = new CustomEvent("tagSelectionChange", {
            detail: {
              selectedTags: this.getSelectedTags(container),
      ...
      console.error("TagDisplay._bindSelectorEvents: 绑定事件失败", error);
    }
  },

  // 私有方法：过滤标签选项
  _filterTagOptions: function (container, searchTerm) {
    try {
      const options = container.querySelectorAll(".tag-option");
      options.forEach((option) => {
        const tagBadge = option.querySelector(".tag-badge");
        const tagText = tagBadge.textContent.toLowerCase();
      ...
      console.error("TagDisplay._filterTagOptions: 过滤失败", error);
    }
  },

  // 私有方法：显示创建标签对话框
  _showCreateTagDialog: function (onSuccess) {
    try {
      if (
        typeof TaskPixel.TagSelector !== "undefined" &&
        TaskPixel.TagSelector.showCreateTagDialog
      ) {
        // 使用现有的创建标签对话框
        TaskPixel.TagSelector.showCreateTagDialog(onSuccess);
      } else {
        // 简单的提示用户手动创建
      ...
      console.error("TagDisplay._showCreateTagDialog: 显示对话框失败", error);
    }
  },
