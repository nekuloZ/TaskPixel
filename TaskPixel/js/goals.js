...
TaskPixel.Goals = {
  ...
  cacheElements: function () {
    ...
    this.elements.priorityEvaluationDialog = document.getElementById(
      "priority-evaluation-dialog"
    );
    this.elements.goalSelectionContainer = document.getElementById(
      "goal-selection-container"
    );
    this.elements.startPriorityEvaluationButton = document.getElementById(
      "start-priority-evaluation"
    );
    this.elements.cancelPriorityEvaluationButton = document.getElementById(
      "cancel-priority-evaluation"
    );
    this.elements.clearTagsButton =
  ...
  bindEvents: function () {
    ...
    if (this.elements.aiPriorityButton) {
      this.elements.aiPriorityButton.addEventListener(
        "click",
        this.openPriorityEvaluationDialog.bind(this)
      );
    }

    // 开始优先级评估按钮事件
    if (this.elements.startPriorityEvaluationButton) {
      this.elements.startPriorityEvaluationButton.addEventListener(
        "click",
        this.startPriorityEvaluation.bind(this)
      );
    }

    // 取消优先级评估按钮事件
    if (this.elements.cancelPriorityEvaluationButton) {
      this.elements.cancelPriorityEvaluationButton.addEventListener(
        "click",
        this.closePriorityEvaluationDialog.bind(this)
      );
    }

    // 清除标签按钮事件
    if (this.elements.clearTagsButton) {
      this.elements.clearTagsButton.addEventListener(
        "click",
        this.clearTagFilter.bind(this)
      );
    }
  ...
  renderGoals: function (goals) {
    ...
    // 应用排序
    const sortMethod = this.elements.sortSelect
  ...
  createGoalElement: function (goal) {
    ...
    goalElement.className =
      "border-4 border-border-color p-4 bg-white shadow-pixel-sm goal-item";
    ...
    goalElement.innerHTML = `
            ...
            <div class="mt-4 flex justify-end">
                <a href="task_detail.html?id=${
                  goal.taskId
                }" class="pixel-button-sm bg-primary text-white font-display py-1 px-2 text-xs flex items-center">
  ...
  renderTags: function (tags) {
    ...
    sortedTags.forEach((tag) => {
      ...
      tagElement.className = `px-3 py-1 rounded ${
        isSelected ? "bg-primary text-white" : "bg-gray-100 text-gray-800"
      } text-sm tag-filter-button`;
  ...
  renderGoalSelections: function () {
    ...
    tasks.forEach((task) => {
      ...
      task.goals.forEach((goal) => {
        ...
        goalItem.className =
          "flex items-center p-3 border-2 border-gray-300 rounded mb-2";

        goalItem.innerHTML = `
                    <input type="checkbox" name="selected-goal" value="${
                      goal.id
                    }" data-task-id="${task.id}" class="mr-3">
                    <div>
                        ...
                        ${
                          goal.priority
                            ? `
                            <span class="inline-block px-2 py-1 text-xs ${this.getPriorityClass(
                              goal.priority
                            )} mt-1">
                                ${this.getPriorityText(goal.priority)}
                            </span>
  ...
  startPriorityEvaluation: async function () {
    ...
    const selectedInputs = document.querySelectorAll(
      'input[name="selected-goal"]:checked'
    );
    ...
    // 显示加载状态
    const removeLoadingMessage = this.showLoadingMessage(
      "AI正在分析目标，生成优先级比较问卷..."
    );

    try {
      // 调用AI辅助模块生成比较问卷
      const questions = await TaskPixel.AI.generateComparisonQuestionnaire(
        selectedGoals
      );
  ...
  showComparisonQuestionnaireDialog: function (selectedGoals, questions) {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      const removeLoadingMessage =
        this.showLoadingMessage("AI正在评估目标优先级...");
  ...
  showComparisonResultDialog: function (result) {
    ...
    // 构建结果内容HTML
    const rankingsHtml = result.rankings
      .map((item, index) => {
                ...
                <div class="border-2 border-gray-200 p-4 ${
                  index === 0 ? "bg-yellow-50" : "bg-white"
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            <h4 class="font-bold text-lg">${index + 1}. ${
          item.title
        }</h4>
  ...
  showSuccessMessage: function (message) {
    ...
    messageElement.className =
      "fixed bottom-4 right-4 bg-accent-green text-white px-4 py-2 rounded shadow-lg pixel-border z-50";
  ...
  showErrorMessage: function (message) {
    const messageElement = document.createElement("div");
    messageElement.className =
      "fixed bottom-4 right-4 bg-accent-red text-white px-4 py-2 rounded shadow-lg pixel-border z-50";
    messageElement.textContent = message;
  ...
  showLoadingMessage: function (message) {
    ...
    messageElement.className =
      "fixed bottom-4 right-4 bg-gray-800 text-white px-4 py-2 rounded z-50 pixel-border";
